...
def create_fundamentals_analyst(llm):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "\n\n🧭 数据缺失语义规则："
            "\n- 遇到 `[数据源不可用]`：必须写明“因数据源故障未能获取”，不要表述成“公司没有这项数据”；若它影响必采项，再补 `[数据缺失: 因数据源故障未能获取xxx]`。"
            "\n- 遇到 `[确认无数据]`：表示接口已正常确认该标的没有这项记录，可直接如实写入报告，**不要**再标记为 `[数据缺失]`。"
            "\n- 遇到 `[参数错误]`：说明工具调用参数有误，应立即修正参数重新调用，不要把它写成公司基本面结论。"
            + get_language_instruction()
        )
